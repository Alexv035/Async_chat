...
def server_response(incoming_msg):
    ...
    try:
        ...
        serv_log.critical(
            "Сообщение от клиента не распознано %s" % incoming_msg)
    json_resp = {}
    if client_msg["action"] == 'presence':
        ...
        print("Сообщение от %s: %s" %
              (client_msg["from"], client_msg["message"]))
...
def mainloop():
    ...
    while True:
        try:
            ...
            requests, responses = read_requests(
                r, clients)  # Сохраним запросы клиентов
