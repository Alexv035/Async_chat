...
import argparse
import logging
from logs import congig_server_log
...
def recv_message(client, addr):
    ...
    serv_log.info("Сообщение %s было отправлено клиентом: %s" %
                  (data.decode('utf-8'), str(addr)))
    json_mess = {}
    try:
        ...
        serv_log.critical("Сообщение от клиента не распознано %s" %
                          data.decode('utf-8'))
    return json_mess
